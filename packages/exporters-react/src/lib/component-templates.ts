...
function generateTextInput(name: string): string {
  return joinLines([
    ...
    `}`,
    ``,
    `export function ${name}({ id, placeholder = 'enter placeholder here', required = false, value = '', onChange }: ${name}Props) {`,
    `  return (`,
...
function generateLineChart(name: string): string {
  return joinLines([
    ...
    ``,
    `  return (`,
    `    <section className="rd-chart-line" id={id} data-testid="rd-chart-line" role="img" aria-label={title}>`,
    `      <header className="rd-chart-line__header"><span>{title}</span></header>`,
    `      <svg className="rd-chart-line__svg" viewBox="0 0 320 120" aria-hidden="true">`,
    `        <polyline`,
    `          className="rd-chart-line__line"`,
    `          fill="none"`,
    `          stroke="currentColor"`,
    `          strokeWidth="2"`,
    `          points={points`,
    ...
    `        />`,
    `      </svg>`,
    `      <p className="rd-chart-line__meta">{points.length} points · x={xField} · y={yField}</p>`,
    `    </section>`,
    `  );`,
    `}`,
    ``,
...
function generateBarChart(name: string): string {
  return joinLines([
    ...
    ``,
    `  return (`,
    `    <section className="rd-chart-bar" id={id} data-testid="rd-chart-bar" role="img" aria-label={title}>`,
    `      <header className="rd-chart-bar__header"><span>{title}</span></header>`,
    `      <div className="rd-chart-bar__bars" aria-hidden="true">`,
    `        {points.map((row, index) => (`,
    `          <div`,
    `            key={String(row.id ?? index)}`,
    `            className="rd-chart-bar__bar-wrap"`,
...
function generateRoleGate(name: string): string {
  return joinLines([
    ...
    `  const rootClass = ['rd-role-gate', visible ? 'rd-role-gate--visible' : 'rd-role-gate--hidden'].join(' ');`,
    ``,
    `  return (`,
    `    <section className={rootClass} id={id} data-testid="rd-role-gate">`,
    `      {label ? <span className="rd-field__label">{label}</span> : null}`,
    `      {visible ? (`,
    `        <>`,
    `          <p className="rd-role-gate__status" data-testid="rd-role-gate-visible">{statusText}</p>`,
    `          {children ?? <p>Protected content</p>}`,
    `        </>`,
    `      ) : (`,
    `        <p className="rd-role-gate__status rd-role-gate__status--hidden" data-testid="rd-role-gate-hidden">`,
    `          {hiddenStatusText}`,
    `        </p>`,
    `      )}`,
    `    </section>`,
...
export function generateScrollRegion(name: string): string {
  return joinLines([
    `'use client';`,
    ``,
    ...
    `  children,`,
    `}: ${name}Props) {`,
    `  const rootClass = ['rd-scroll-region', overlayScrollbar ? 'rd-scroll-region--overlay-scrollbar' : '', className]`,
    `    .filter(Boolean)`,
    `    .join(' ');`,
    `  const regionStyle: CSSProperties = { ...style, ...(maxHeight ? { maxHeight } : {}) };`,
    ``,
    `  return (`,
    `    <section`,
    `      id={id}`,
